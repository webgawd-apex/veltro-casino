...
export const generateCrashPoint = (serverSeed, clientSeed, nonce, players = []) => {
  ...
  const significantRealBet = realPlayers.some(p => p.amount >= 0.01);
  
  // House edge only triggers if real players are betting significant amounts
  const houseWins = significantRealBet && Math.random() < 0.70;

  if (houseWins) {
    console.log(`[HOUSE EDGE] House wins this round (70% trigger for real players).`);
