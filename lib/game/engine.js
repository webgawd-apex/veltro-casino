...
const CLIENT_SEED = "HateCasino-MVP-Phase-1"; // Public seed
...
const runLifecycle = async () => {
  ...
  console.log(`[STATUS] BETTING - 10s for new bets - Round #${nonce}`);
  
  await new Promise(resolve => setTimeout(resolve, BETTING_DURATION));

  // 2. RUNNING State
  const crashPoint = generateCrashPoint(SERVER_SEED, CLIENT_SEED, nonce);
