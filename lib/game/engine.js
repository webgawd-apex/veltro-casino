...
import { clearPlayers, getPlayers, addSimulatedPlayers } from './players.js';
import { generateCrashPoint } from './crash.js';
import { processAutoCashouts, markBustedPlayers, processManualCashout } from './payouts.js';
import { generateSimulatedBatch, getSimulatedCashouts } from './simulatedPlayers.js';
...
const CLIENT_SEED = "Veltro-Crash-Mainnet"; // Public seed
...
const runLifecycle = async () => {
  ...
  console.log(`[STATUS] BETTING - 20s for new bets - Round #${nonce}`);

  // Inject simulated players halfway through betting
  setTimeout(() => {
    const simCount = Math.floor(Math.random() * 5) + 2; // 2-6 simulated players
    const simPlayers = generateSimulatedBatch(simCount);
    addSimulatedPlayers(simPlayers);
    if (io) io.emit("playersUpdate", getPlayers());
    console.log(`[SIM] Injected ${simCount} simulated players.`);
  }, 10000);
  ...
  return new Promise(resolve => {
    gameInterval = setInterval(() => {
      ...
      if (multiplier >= crashPoint) {
        ...
        // Handle simulated player cashouts
        const simCashouts = getSimulatedCashouts(getPlayers(), multiplier, crashPoint);
        simCashouts.forEach(wallet => {
          processManualCashout(wallet, multiplier, io);
        });
      }
    }, TICK_RATE);
  });
