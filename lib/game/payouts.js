...
const connection = new Connection(process.env.RPC_URL || "https://solana-mainnet.core.chainstack.com/50d9fbef13c14089c59929338f006803", "confirmed");
...
export const processAutoCashouts = (currentMultiplier, io) => {
  ...
  players.forEach((p) => {
    if (p.status === 'playing' && p.target && currentMultiplier >= p.target) {
      ...
      const account = accountsModule.creditBalance(p.wallet, winAmount);
      accountsModule.addBetHistory(p.wallet, {
        game: 'Crash',
        multiplier: p.target,
        profit: p.profit,
        amount: p.amount
...
export const processManualCashout = (wallet, currentMultiplier, io) => {
  ...
  if (player && player.status === 'playing') {
    ...
    const account = accountsModule.creditBalance(wallet, winAmount);
    accountsModule.addBetHistory(wallet, {
      game: 'Crash',
