...
if (!process.env.DATABASE_URL) {
  throw new Error("[DB] DATABASE_URL is not defined in environment variables. Please set it in your .env or Render dashboard.");
}
...
export const initDB = async (retries = 5) => {
  ...
  // Retry loop for serverless DB wake-up (Neon)
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.warn(`[DB] Connection attempt ${i + 1} failed (database might be waking up). Retrying in 2s...`);
