...
export default function Header() {
  ...
  const { isMobile, mobilePublicKey, connectMobile, disconnectMobile } = useMobileWallet();
  const [balance, setBalance] = useState(0);
  const [casinoBalance, setCasinoBalance] = useState(0);
  const [drawerOpen, setDrawerOpen] = useState(false);
  const pathname = usePathname();

  // Use adapter key on desktop, mobile deep-link key on mobile
  const publicKey = adapterPublicKey || (mobilePublicKey ? { toBase58: () => mobilePublicKey } : null);
  ...
  return (
    <>
        ...
        <header className="glass border-b border-white/5 px-4 md:px-6 py-3 md:py-4 flex items-center justify-between relative z-20">
          ...
          <div className="flex items-center justify-end gap-3 md:gap-5 lg:w-1/3">
            ...
            <div className="relative">
              ...
              {!publicKey && (
                isMobile ? (
                  // Mobile: deep link to Phantom, browser stays as HQ
                  <button
                    onClick={connectMobile}
                    className="h-9 px-4 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-purple-900/30 hover:from-purple-500 hover:to-indigo-500 active:scale-95 transition-all flex items-center gap-2"
                  >
                  ...
                  // Desktop: standard wallet adapter popup
      ...
      <ProfileDrawer
        ...
        disconnectMobile={disconnectMobile}
        isMobile={isMobile}
      />
    </>
  );
